...
This fork is used to add and maintain LoongArch64 support and to build
LoongArch64 releases.

- Follow every instruction from the upstream `AGENTS.md` text below.
- LoongArch64 validation may use the container image
  `ghcr.io/zarraxx/debian:trixie`.
- LoongArch64 validation may also use real hardware reachable with
  `ssh loongarch`.
- To keep syncing from upstream easy, do not directly modify upstream files in
  this fork.
- Put fork-specific changes under the `fork_patches/` directory. This includes
  patch files, scripts, metadata, and notes used to apply or maintain
  LoongArch64 support.
- When a task appears to require changing an upstream file, create or update the
  corresponding patch or support file under `fork_patches/` instead.
